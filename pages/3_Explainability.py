...
def normalise_feature_attribution(
    ...
    if not required_columns.issubset(
        ...
        raise ValueError(
            "Feature attribution file must contain "
            "feature and attribution columns."
        )
...
def normalise_event_attribution(
    ...
    """
    Standardise TimeSHAP or Integrated Gradients
...
# ============================================================
# Standardise loaded XAI artefacts
# ============================================================

timeshap_feature_df = (
    normalise_feature_attribution(
        timeshap_feature_df
    )
)
...
section_title(
    "1. Direct LSTM Explainability Overview"
)

st.markdown(
    """
    The final Improved LSTM is explained directly using
    two complementary post-hoc Explainable Artificial
    Intelligence techniques.

    TimeSHAP provides sequence-specific feature,
    event and coalition explanations, while Integrated
    Gradients provides independent feature, timestep
    and sensor-time attribution.
    """
)
...
st.info(
    """
    Both explanation methods operate directly on the
    Improved LSTM used for Remaining Useful Life
    prediction.

    The Tuned Random Forest is retained only as an
    interpretable predictive baseline within the
...
if st.session_state.get(
    ...
    engine_id = st.session_state.get(
        "prediction_engine_id",
        None,
    )

    start_cycle = st.session_state.get(
        "prediction_start_cycle",
        None,
    )

    end_cycle = st.session_state.get(
    ...
    st.markdown(
        ...
        **Engine ID:** {engine_id if engine_id is not None else "Not supplied"}  
        **Operational Window:** {
    ...
    if st.session_state.get(
        ...
        st.success(
            ...
            **Representative XAI Case Detected**
            ...
            The current prediction corresponds to the
            representative Critical Engine 41 sequence
            used during the validated Explainable AI
            analysis.

            The TimeSHAP and Integrated Gradients
            artefacts presented below therefore relate
            to this representative engine case.
            """
        )
        ...
        st.warning(
            """
            **Different Prediction Case**

            The current prediction does not correspond
            to the representative Critical Engine 41
            case used to generate the saved TimeSHAP
            and Integrated Gradients artefacts.

            The explanation results below remain valid
            experimental artefacts for the representative
            Critical case, but they should not be
            interpreted as explanations of the current
            prediction.
            """
        )
    ...
    st.info(
        """
        No prediction has been generated in the current
        Streamlit session.

        The Explainability module below presents the
        validated TimeSHAP and Integrated Gradients
        analysis for the representative Critical
        Engine 41 case.
        """
    )
...
st.markdown(
    """
    TimeSHAP is the primary explanation method because
    it was developed specifically for recurrent sequence
    models.

    The chart below identifies the sensor and operational
    variables that most strongly influenced the
    representative LSTM prediction.
    """
)

max_timeshap_features = len(
    timeshap_feature_df
)
...
st.info(
    ...
    **{", ".join(top_timeshap_features)}**.
    ...
    Negative attributions reduce the predicted RUL
    relative to the healthy operating baseline, while
    positive attributions increase it.
    """
)
...
section_title(
    "3. TimeSHAP Event and Coalition Explanation"
)

st.markdown(
    """
    Event attribution indicates how individual
    observations within the 30-cycle sequence influenced
    the LSTM prediction.

    Coalition pruning evaluates whether earlier events
    can be removed without materially changing the
    explanation.
    """
)

timeshap_event_plot_df = (
    timeshap_event_df.copy()
)
...
for candidate in [
    ...
    if candidate in (
        timeshap_event_plot_df.columns
    ):
...
if (
    pruning_index is None
    and not timeshap_pruning_df.empty
):

    pruning_message = (
        "The pruning analysis was successfully "
        "generated. Review the table below for "
        "coalition-level results."
    )
...
st.success(
    ...
    **Coalition pruning interpretation**
    ...
    A value of **−30** indicates that the complete
    30-cycle observation window contributed to the
    explanation and that no historical events could
    be removed under the selected tolerance.
    """
)
...
st.markdown(
    ...
    gradient-based explanation of the same Improved
    LSTM.

    Its feature and timestep attributions are used to
    validate whether TimeSHAP identifies consistent
...
st.markdown(
    """
    The sensor-time heatmap shows how each selected
    feature influenced the LSTM prediction at every
    timestep within the 30-cycle input window.

    This reveals both which sensors mattered and when
    their influence became strongest.
    """
)
...
if ig_sensor_time.shape != (
    ...
    st.warning(
        (
            "The sensor-time matrix shape does not "
            "exactly match the expected 30 × feature "
            "configuration. The heatmap will use the "
...
heatmap_figure.update_layout(
    title=(
        "Integrated Gradients Sensor-Time "
        "Attribution for the 30-Cycle Window"
    ),
    xaxis_title=(
        "Relative timestep "
...
if (
    ...
    st.warning(
        ...
        **Critical degradation window**
        ...
        The highest cumulative temporal attribution
        occurred between operational cycles
        **{critical_start} and {critical_end}**.

        This interval represents the period that
        contributed most strongly to the representative
        Critical engine prediction.
        """
    )
...
section_title(
    "6. Explanation Faithfulness and Agreement"
)

st.markdown(
    """
    Explanation quality is evaluated through feature
    deletion and cross-method ranking agreement.

    Masking highly ranked variables with healthy-reference
    values should materially change the LSTM prediction
    if the explanations are faithful.
    """
)
...
fidelity_chart = px.line(
    ...
    markers=True,
    title=(
        "Feature-Deletion Faithfulness Test"
    ),
    labels={
        "fraction_masked":
            "Fraction of top features masked",
...
st.info(
    """
    The very high TimeSHAP–Integrated Gradients
    rank correlation demonstrates strong agreement
    between two theoretically different explanation
    methods.

    The lower baseline correlation confirms that
    Integrated Gradients explanations depend on the
    reference state and supports the use of a
    physically meaningful healthy baseline rather
    than an all-zero sequence.
    """
)
...
st.markdown(
    """
    The engine-level panel links each predicted RUL
    to its health status, risk level and recommended
    maintenance action.

    This panel uses the validated dashboard prediction
    dataset generated during the experimental phase.
    """
)
...
# Default to Engine 41 when available
default_engine_index = 0

if 41 in engine_ids:

    default_engine_index = list(
        engine_ids
    ).index(41)
...
st.info(
    ...
    **Engine {int(selected_engine)} decision explanation**
    ...
    The validated experimental Improved LSTM prediction
    is approximately
    **{engine_record['predicted_rul_lstm']:.2f}
...
st.markdown(
    """
    This section retains the original interactive
    sensor-trend analysis as descriptive engineering
    context.

    Unlike TimeSHAP and Integrated Gradients, these
    plots visualise sensor behaviour but do not
    independently attribute the LSTM prediction.
    """
)
...
temporal_default_index = 0

if 41 in available_temporal_engines:

    temporal_default_index = list(
        available_temporal_engines
    ).index(41)
...
if selected_temporal_features:
    ...
    temporal_chart = px.line(
        ...
        title=(
            ...
            f"Engine "
            f"{int(selected_temporal_engine)}"
        ),
    )
    ...
    st.info(
        ...
        **Engineering interpretation**
        ...
        For Engine
        **{int(selected_temporal_engine)}**:

        - {"; ".join(trend_summaries)}.

        These descriptive trends provide
        operational context for the
...
st.success(
    ...
    **Final XAI decision**
    ...
    TimeSHAP is adopted as the primary
    sequence-aware explanation technique,
    while Integrated Gradients provides
    independent feature, temporal and
    sensor-time validation.

    Together, both methods explain the
    Improved LSTM and support transparent,
    human-centred predictive maintenance
    decision-making.
    """
)
